...
def test_multiple_vars(tmp_path):
    org = _org_scope(tmp_path, {
        "testing.md": "Target: {{ coverage_target }}\nFramework: {{ framework }}",
    })
    config = Config(scopes=[
        org,
        Scope(name="api", repos=["api-gw"],
              vars={"coverage_target": "90%", "framework": "pytest"}),
    ])
    hierarchy = resolve_hierarchy(config, "x/api-gw")
...
def test_vars_apply_to_all_files(tmp_path):
    ...
    config = Config(scopes=[
        org,
        Scope(name="api", repos=["api-gw"],
              vars={"coverage_target": "90%", "team_name": "API"}),
    ])
    hierarchy = resolve_hierarchy(config, "x/api-gw")
    merged = merge_content(hierarchy)
    assert "90%" in merged.get("testing.md").content
...
def test_different_repos_get_different_vars(tmp_path):
    ...
    assert "90%" in merge_content(
        resolve_hierarchy(config, "x/api-gw")).get("testing.md").content
    assert "75%" in merge_content(
        resolve_hierarchy(config, "x/web-app")).get("testing.md").content
...
def test_non_matching_repo_no_vars(tmp_path):
    ...
    content = merge_content(
        resolve_hierarchy(config, "x/other-repo")).get("testing.md").content
...
def test_vars_work_with_append_strategy(tmp_path):
    ...
    config = Config(scopes=[
        org,
        Scope(name="api", repos=["api-gw"], strategy="append",
...
def test_no_vars_leaves_content_unchanged(tmp_path):
    ...
    config = Config(scopes=[org])
    content = merge_content(
        resolve_hierarchy(config, "x/any-repo")).get("testing.md").content
